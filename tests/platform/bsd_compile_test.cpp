//
// bsd_compile_test.cpp
// ~~~~~~~~~~~~~~~~~~~~
//
// Copyright (c) 2023 Jack (jack.arain at gmail dot com)
//
// Distributed under the Boost Software License, Version 1.0. (See accompanying
// file LICENSE_1_0.txt or copy at http://www.boost.org/LICENSE_1_0.txt)
//
//

// kqueue 后端的编译期检查：本地没有 BSD 系统头文件时使用 tests/platform/stub
// 下的桩接口，保证这一份代码不会因为长期不被编译而失效。

#include "watch_service_interface.hpp"

#include <watchman/bsd/bsd_watchman.hpp>

#include <boost/asio/any_io_executor.hpp>
#include <boost/asio/io_context.hpp>
#include <boost/asio/use_future.hpp>
#include <boost/filesystem.hpp>

#include <type_traits>

namespace {

	namespace net = boost::asio;
	namespace fs = boost::filesystem;

	using bsd_watch = watchman::bsd_watch;

	static_assert(std::is_same_v<bsd_watch,
		watchman::bsd_watch_service<net::any_io_executor>>);

	static_assert(watchman::test::watch_service_interface<bsd_watch>);

	static_assert(bsd_watch::supported_cancellation ==
		(net::cancellation_type::terminal | net::cancellation_type::total));

	using concrete_watch =
		bsd_watch::rebind<net::io_context::executor_type>::other;

	static_assert(watchman::test::watch_service_interface<concrete_watch>);

	// 该函数不会被调用，只为实例化各平台的等待入口。
	void use_bsd_watch(const fs::path& dir)
	{
		net::io_context io;
		bsd_watch watch(io.get_executor(), dir);

		watch.async_wait([](boost::system::error_code, watchman::notify_events)
			{
			});

		watch.async_wait(net::use_future);
		watch.cancel();
		watch.close();
	}

	// 协程 token 同样可以作为完成令牌。
	[[maybe_unused]] void await_bsd_watch(bsd_watch& watch)
	{
		static_cast<void>(watchman::test::await_watch_events(watch));
	}
} // namespace

int main()
{
	use_bsd_watch(fs::path{});
	return 0;
}
